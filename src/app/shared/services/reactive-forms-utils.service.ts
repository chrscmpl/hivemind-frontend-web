...
export class ReactiveFormsUtilsService {
  ...
  private markGroupOrArrayAs(form: FormGroup | FormArray, markAs: Mark): void {
    if (form instanceof FormGroup)
      for (const controlName of Object.keys(form.controls ?? {})) {
        this.markAllAs(form.get(controlName) as AbstractControl, markAs);
      }
    else if (form instanceof FormArray)
      for (const control of form.controls) this.markAllAs(control, markAs);
  }
